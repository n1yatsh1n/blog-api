...
export function signToken(u: UserSafe): string {
  return jwt.sign({ id: u.id, email: u.email, username: u.username }, JWT_SECRET, { expiresIn: "7d" });
}

export async function requireAuth(req: Request, res: Response, next: NextFunction) {
  try {
    const header = req.headers["authorization"];
    if (!header) return res.status(401).json({ error: { message: "Authorization header is missing" } });
    const [type, token] = header.split(" ");
    if (type !== "Bearer" || !token) return res.status(401).json({ error: { message: "Invalid Authorization header format" } });
