...
export function setupSwagger(app: Express) {
  const spec = {
    ...
    info: {
      ...
      version: "1.0.0",
      description: "RESTful API for a simplified blog platform (Node.js + Express + Sequelize)."
    },
    servers: [{ url: "/" }],
    components: {
      securitySchemes: {
    ...
    paths: {
      "/api/users": {
        post: {
          ...
          responses: {
            "201": { description: "Created", content: { "application/json": { schema: { type: "object", properties: { user: { $ref: "#/components/schemas/UserAuth" } } } } } }
          }
        }
      },
      "/api/users/login": {
